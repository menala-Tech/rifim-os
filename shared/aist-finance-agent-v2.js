(function (global) {
  ...
  function currentRole() {
    try {
      ...
      return String((raw && JSON.parse(raw).role) || '').toLowerCase()
    } catch (_) { return '' }
  }
  ...
  async function requestManual(button) {
    if (!canWrite()) {
      if (typeof showToast === 'function') showToast('Role ini read-only untuk AIST Auto-Fill', 'err')
      return
    }
    ...
    try {
      ...
      if (!res.ok || data.success !== true) throw new Error(data.message || 'Gagal kirim job ke AIST Agent')
      button.textContent = '🟡 Agent Processing'
      if (typeof showToast === 'function') showToast('✅ Job dikirim ke MENALA AIST Agent', 'ok')
      setTimeout(function () { if (typeof loadSaldoRaos === 'function') loadSaldoRaos() }, 2500)
    } catch (err) {
      button.disabled = false
      button.textContent = original
      if (typeof showToast === 'function') showToast('❌ ' + (err && err.message ? err.message : String(err)), 'err')
    }
  }
