(function (global) {
  ...
  async function onGenerated(code, payload, result) {
    ...
    const employeeId = String(payload?.employee_id || payload?.extra?.employee_id || '').trim()
    ...
    const body = {
      ...
      gdoc_url: pick(result, 'gdoc_url', 'google_doc_url', 'doc_url', 'gdocUrl'),
      pdf_url: pick(result, 'pdf_url', 'pdfUrl'),
      document_id: pick(result, 'document_id', 'id'),
      start_date: payload.contract_start || payload?.extra?.contract_start || null,
      end_date: payload.contract_end || payload?.extra?.contract_end || null,
      payload,
    }
