const ALLOWED_ROLES = new Set(['admin', 'management', 'direksi'])
...
async function verifyActor(token, supabaseUrl, publishableKey) {
  ...
  const profileRes = await fetch(
    `${supabaseUrl}/rest/v1/user_profiles?id=eq.${encodeURIComponent(authUser.id)}&select=id,role,is_active&limit=1`,
    {
      headers: {
        apikey: publishableKey,
        Authorization: `Bearer ${token}`,
  ...
  if (!ALLOWED_ROLES.has(role)) throw new Error('Akses AIST hanya admin/management/direksi')
...
async function runPlaywright(runnerUrl, runnerSecret, payload) {
  ...
  if (!response.ok || data?.success !== true) {
    throw new Error(data?.message || data?.error || `AIST runner gagal (HTTP ${response.status})`)
  }
  return data
}
...
async function markPaid(gasUrl, token, requestId) {
  const response = await fetch(gasUrl, {
    ...
    body: JSON.stringify({
      action: 'finance_saldo_raos_mark_paid',
      access_token: token,
      id: requestId,
    }),
  })
  const data = await readJson(response)
  if (!response.ok || data?.success !== true) {
    throw new Error(data?.message || `Mark-paid gagal (HTTP ${response.status})`)
  }
  if (data.status && !['updated', 'already_processed'].includes(data.status)) {
    throw new Error(`Mark-paid ditolak: ${data.status}`)
  }
  return data
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const runner = await runPlaywright(runnerUrl, runnerSecret, {
      ...
      driver_login: input.driverLogin,
      nominal: input.nominal,
    })

    // Mark paid HANYA setelah runner mengonfirmasi AIST sukses.
    ...
    return json(res, 200, {
      ...
      runner: {
        ...
        completed_at: runner.completed_at || null,
      },
