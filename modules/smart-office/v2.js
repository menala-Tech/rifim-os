...
(function(){
  ...
  async function post(action,extra){
    ...
    const a=auth();
    if(!a.access_token)throw new Error('Session Portal tidak memiliki access token. Login ulang.');
    payload.access_token=a.access_token;
    payload.performed_by={name:a.full_name||a.name||'',email:a.email||''};
  ...
  function render(){
    ...
    const submit=q('soV2Submit'); if(submit)submit.disabled=!canWrite()||approvalStatus==='pending_approval'||approvalStatus==='approved';
    ...
    const reject=q('soV2Reject'); if(reject){reject.style.display=isDireksi()?'inline-flex':'none';reject.disabled=!lastApprovalId||approvalStatus!=='pending_approval';}
  ...
  window.soV2SubmitApproval=async()=>{
    try{await ensureDraft();if(approvalStatus==='rejected'){const rev=await post('so_revise',{documentId});approvalStatus=rev.status||'draft';}const r=await post('so_submit',{documentId});approvalStatus=r.status||'pending_approval';render();showToast('📨 Pengajuan dikirim ke Direksi',true);}
    catch(e){showToast('❌ '+e.message,false);}
  };
  ...
  window.soV2Reject=async()=>{
    try{if(!lastApprovalId)throw new Error('Approval pending belum dipilih.');const r=await post('so_decide',{approvalId:lastApprovalId,decision:'rejected'});approvalStatus=r.documentStatus||'rejected';render();showToast('↩ Dokumen ditolak dan harus direvisi',false);}
    catch(e){showToast('❌ '+e.message,false);}
  };
  ...
  function mount(){
    const a=auth();
    if(!a.access_token){window.location.href='/portal';return;}
    const actions=document.querySelector('.form-actions');if(!actions||q('soV2Workflow'))return;
...
<button id="soV2Reject" class="so-v2-btn so-v2-reject" onclick="soV2Reject()">↩ Tolak</button>\
...
<button id="soV2Chat" class="so-v2-btn so-v2-chat" onclick="soV2SendChat()">💬 Kirim ke RAOS Chat</button>';
  ...
  window.generateDoc=function(){showToast('Gunakan workflow: Preview → Ajukan ke Direksi → Generate setelah approved.',false);};
  window.previewToGenerate=function(){if(typeof closePreview==='function')closePreview();window.soV2SubmitApproval();};
  window.doLogin=function(){window.location.href='/portal';};
